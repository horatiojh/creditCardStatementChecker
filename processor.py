...
csv = '/Users/horatio/Coding/pyScripts/creditCardStatementChecker/resources/transactions2506.csv'
month = 6 # calculates that month's statement, from 16 of prev month to 15 of that month
year = 2022
...
# get cashback amount credited to acc (cashback from previous month is credited in current bill)
# FYI: it seems to have changed to 52FrosJ21Cashback30
# idxmax returns the row number of the first occurence of "30CASHBACK"
...
# remove the unposted transactions (but we'll also have to account for this in the next month's bill... I think)
monthTransactions = monthTransactions[monthTransactions['DESCRIPTION'].str.contains("UNPOSTED") == False]
...
cashbackAmt = 0.02* monthTransactions.loc[monthTransactions['SGD Amount'] > 0, 'SGD Amount'].sum()
...
if owedAmt != amtDue:
    print("The statement difference is $" + str(owedAmt - amtDue))
    print(colored("The statement does not tally", 'red'))
    print("There could be a transaction that wasn't posted in time. The about being asked for should be less than that being owed. The Youtube membership usually also gets posted too late ($15)")
else:
